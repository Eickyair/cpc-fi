...
int upperBound_suffix(string &s,string &sub,vi &suffix)
{
    int l = -1;    // a[l] <= x
    int r = sz(suffix); // a[r]>x
    string cmp;
    while(r>l+1){
        int m = (r + l) / 2;
        cmp = s.substr(suffix[m], sz(sub));
        if(cmp<sub){
            l = m;
        }else{
            r = m;
...
int lowerBound_suffix(string &s,string &sub,vi &suffix){
    int l = -1;    // a[l] <= x
    int r = sz(suffix); // a[r]>x
    string cmp;
    while(r>l+1){
...
int main() {
    ...
    while(n--){
        ...
        cin >> sub;
        // debug(sub);
        int init = upperBound_suffix(s, sub, suffix);
        int end = lowerBound_suffix(s, sub, suffix);
        int cnt = end - init+1;
        cout << cnt << endl;
    }
    return 0;
